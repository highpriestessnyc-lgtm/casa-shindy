...
export default async function LessonPage() {
  ...
  const { data: lessons } = await supabase.from('lessons').select('*').eq('is_published', true).order('created_at', { ascending: false })
  return (
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
          {lessons?.map((lesson: any) => (
            <div key={lesson.id} style={{ background:'#111', border:'1px solid rgba(255,255,255,0.07)', padding:'2rem' }}>
              <div style={{ fontSize:'0.6rem', color:'rgba(248,246,242,0.3)', marginBottom:'0.8rem' }}>{new Date(lesson.created_at).toLocaleDateString('ja-JP')}</div>
          ...
          {!lessons?.length && <div style={{ textAlign:'center', padding:'4rem', color:'rgba(248,246,242,0.3)' }}>まだレッスンがありません</div>}
        </div>
      </div>
    </main>
  )
