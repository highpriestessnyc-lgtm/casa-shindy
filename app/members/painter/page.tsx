          ...
          {artworks?.map((art: any) => (
            <div key={art.id} style={{ position:'relative' }}>
              ...
              <div style={{ 
                ...
                boxShadow:'0 0 0 2px #c9a96e, 0 0 0 4px #2a1f0e, 0 8px 32px rgba(0,0,0,0.6)',
                background:'#1a1408',
                padding:'12px'
              }}>
                {/* 絵 */}
                  ...
                  {art.is_sold && (
                    <div style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center' }}>
                      <div style={{ 
                        ...
                        textDecorationColor:'#d94030',
                        textDecorationThickness:'3px',
                        background:'rgba(0,0,0,0.5)',
                        padding:'0.5rem 1.5rem',
                        border:'2px solid #d94030'
                      }}>SOLD OUT</div>
                    </div>
                  )}
                </div>
              ...
              <div style={{ marginTop:'1.2rem', textAlign:'center' }}>
                ...
                <div style={{ display:'flex', justifyContent:'center', alignItems:'center', gap:'1rem' }}>
                  <span style={{ fontFamily:'serif', fontStyle:'italic', fontSize:'1.3rem', color: art.is_sold ? 'rgba(248,246,242,0.3)' : '#c9a96e' }}>
                    {art.is_sold ? '———' : `¥${art.price?.toLocaleString()}`}
                  </span>
                  {!art.is_sold && (
                    <a href={`https://instagram.com/ma_painter`} target="_blank" style={{ fontSize:'0.6rem', letterSpacing:'0.2em', textTransform:'uppercase', color:'#080808', background:'#c9a96e', padding:'0.4rem 1rem', textDecoration:'none', fontWeight:'bold' }}>
                      購入希望 →
                    </a>
                  )}
                </div>
