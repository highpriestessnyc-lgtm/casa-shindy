...
const PRODUCTS = [
  ...
  { key:'dance-history', icon:'📖', name:'ダンスの歴史本', desc:'ストリートダンスの歴史書。会員は無料。', price:450, href:'/members/dance-history', type:'link', memberFree:true },
          ...
          {PRODUCTS.map((p) => (
            <div key={p.key} style={{ background: p.featured ? 'linear-gradient(160deg,#1a1008,#161616)' : '#0d0d0d', padding:'2.5rem 2rem', display:'flex', flexDirection:'column', position:'relative', border: p.featured ? '1px solid rgba(201,169,110,0.18)' : 'none' }}>
              ...
              <button onClick={() => handleBuy(p)} disabled={loading === p.key} style={{ background: p.price === 0 ? 'transparent' : 'linear-gradient(135deg,#c9a96e,#e8c98a)', color: p.price === 0 ? '#c9a96e' : '#080808', fontSize:'0.62rem', letterSpacing:'0.3em', textTransform:'uppercase', padding:'0.9rem', border: p.price === 0 ? '1px solid rgba(201,169,110,0.4)' : 'none', cursor:'pointer', fontWeight:'bold' }}>
                {loading === p.key ? '...' : p.price === 0 ? 'Free Download' : p.type === 'link' ? 'Open' : 'Buy'}
              </button>
            </div>
          ))}
        </div>
